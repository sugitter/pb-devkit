...
| 📦 Source Extraction | Extract source from EXE/PBD (zero dependency) |
| 📦 源码提取 | 从 EXE/PBD 提取源码（纯 Python，无需 DLL） |
| 📦 Pack (1.x) | Write .sr* source files back into PBL binary |
| 📦 回写 PBL（1.x） | 将 .sr* 源码文件写回 PBL 二进制（纯 Python） |
| 🔀 Migrate (1.x) | Convert PB DW/events → Angular TypeScript scaffold |
| 🔀 迁移（1.x） | PB DataWindow/事件 → Angular TypeScript 脚手架 |
| 🖥️ Desktop GUI | Tauri + Angular cross-platform desktop app |
| 🖥️ 桌面图形界面 | Tauri + Angular 跨平台桌面应用 |
| 🔧 Doctor | Diagnose Python/Rust environment |
| 🔧 环境诊断 | 检查 Python/Rust 运行环境 |
...
| 🔧 | Doctor / 环境诊断 | Check Python/Rust environment status |
| 📦 | Decompile / 源码提取 | Extract source from EXE/PBD (zero dependency) |
...
## 🔧 Zero-Dependency Architecture / 零依赖架构

**1.x Python version** (in `pb-devkit-1.x/`) — completely DLL-free:

1.x Python 版本（`pb-devkit-1.x/` 目录）— 完全无 DLL 依赖：

```
EXE/PBL  ──pb export──►  .sr* source  (pbl_parser.py, pure Python)
.sr* source ──pb pack──►  .pbl binary  (pbl_writer.py, pure Python)
EXE/PBL  ──pb migrate──►  Angular 18 TypeScript scaffold
Need to compile ──pb build──►  PBGen.exe (requires PB IDE)
```

> No third-party DLL required. Works fully offline.
> 无需任何第三方 DLL，完全离线可用。
...
| `pb-devkit-1.x` (Python) | Python 3.9+ (zero external dependencies) |
